...
function categories()
{
    ...
    $oid = $mysqli->query("SELECT p.product_id, p.product_name, p.price, c.category_id, c.category_name, pi.file_name
                                                FROM product p JOIN category c ON (p.category_id = c.category_id) JOIN product_image pi ON (pi.product_id = p.product_id)
                                                AND pi.type = 'main'
